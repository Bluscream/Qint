...
function log(severity, message) {
	...
	switch (severity) {
		case "error":
			console.error(message, args); return;
		case "warn":
			console.warn(message, args); return;
		case "debug":
			console.debug(message, args); return;
		case "trace":
			console.trace(message, args); return;
		default:
			console.log(message, args); return;
	}
}

function getEvents() {
	let events = localStorage.getItem('events');
	if (events === null) { return JSON.parse("{}"); }
...
function addEvent(event) {
	...
	events[name] = event;
	setEvents(events);
	log("warn", "Added event " + name + " to JSON.parse(localStorage.getItem('events'))");
}
